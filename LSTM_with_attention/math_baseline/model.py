...
class MathBaselineLSTMwAttention(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(self.embed_size + self.hidden_size, self.hidden_size)
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size, len(self.vocab.vocab.char2id), bias = False)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        return combined_outputs.to(self.device)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, 1), enc_hiddens), 1) # (b x 2h)
        U_t = torch.cat((a_t, dec_hidden), dim = 1) # (b x 3h)
        V_t = self.combined_output_projection(U_t) # (b x h)
