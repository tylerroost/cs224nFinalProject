...
from utils import batch_iter, get_size_of_model
...
def compute_corpus_score(references: List[List[str]], hypotheses: List[Hypothesis]) -> float:
...
def train(args):
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            tgt_words_num_to_predict = sum(len(s[1:]) for s in tgt_sents)  # omitting leading `<sos>`
...
def decode(args: Dict[str, str]):
    ...
    test_data = nlp.load_dataset('math_dataset', split = ['test[:10%]'], cache_dir = "D:/.cache/nlp/")
    ...
    score = compute_corpus_score(test_data_tgt, top_hypotheses)
